...
func IntersectSegmentPoly2D(p0 []float32, p1 []float32, verts []float32, nverts int, tmin *float32, tmax *float32, segMin *int, segMax *int) bool {
...
func IntersectSegSeg2D(ap []float32, aq []float32, bp []float32, bq []float32, s *float32, t *float32) bool {
	var u, v, w [3]float32
	Vsub(u[:], aq, ap)
	Vsub(v[:], bq, bp)
	Vsub(w[:], ap, bp)
...
func CalcPolyCenter(tc []float32, idx []int16, nidx int, verts []float32) {
	...
	var s float32 = 1.0 / (float32)(nidx)
	tc[0] *= s
	tc[1] *= s
	tc[2] *= s
}

func projectPoly(axis []float32, poly []float32, npoly int, rmin *float32, rmax *float32) {
	dot := Vdot2D(axis, poly)
	*rmin = dot
	*rmax = dot
...
func overlapRange(amin float32, amax float32, bmin float32, bmax float32, eps float32) bool {
...
func OverlapPolyPoly2D(polya []float32, npolya int, polyb []float32, npolyb int) bool {
	...
	for i, j := 0, npolya-1; i < npolya; i, j = i+1, i {
		va := polya[j*3:]
		vb := polya[i*3:]
	...
	for i, j := 0, npolyb-1; i < npolyb; i, j = i+1, i {
		va := polyb[j*3:]
		vb := polyb[i*3:]
		n := [3]float32{vb[2] - va[2], 0, -(vb[0] - va[0])}
...
func SwapEndianUint16(v *uint16) {
...
func SwapEndianUint32(v *uint32) {
	pv0 := unsafe.Pointer(v)
	pv1 := unsafe.Add(pv0, 1)
...
func RandomPointInConvexPoly(pts []float32, npts int, areas []float32, s float32, t float32, out []float32) {
	// Calc triangle araes
	var areasum float32 = 0.0

	for i := 2; i < npts; i += 1 {
		areas[i] = TriArea2D(pts[0:], pts[(i-1)*3:], pts[i*3:])
		areasum += Max(0.001, areas[i])
	}
	// Find sub triangle weighted by area.
	var thr float32 = s * areasum
	var acc float32 = 0.0
	var u float32 = 0.0
	var tri int = 0
	for i := 2; i < npts; i += 1 {
